...
import { SESSION_TOKEN_HEADER, SESSION_TOKEN_COOKIE } from '../lib/server-auth.js'
...
describe('server-auth-bootstrap', () => {
  it('returns 401 for unauthenticated HTML routes without disclosing the token in body or Set-Cookie', async () => {
    const root = await app.fetch(new Request('http://127.0.0.1/', {
    ...
    const plan = await app.fetch(new Request('http://127.0.0.1/plan/test', {
      headers: { Host: '127.0.0.1' },
    }))
  ...
  it('rejects a hostile Origin with 403 on authenticated HTML and API routes', async () => {
    const html = await app.fetch(new Request('http://127.0.0.1/', {
      headers: { Host: '127.0.0.1', Origin: 'http://evil.test', [SESSION_TOKEN_HEADER]: TOKEN },
    }))
    expect(html.status).toBe(403)

    const api = await app.fetch(new Request('http://127.0.0.1/api/comments', {
      headers: { Host: '127.0.0.1', Origin: 'http://evil.test', [SESSION_TOKEN_HEADER]: TOKEN },
    }))
  ...
  it('serves same-origin authenticated API requests (GET /api/comments)', async () => {
    const api = await app.fetch(new Request('http://127.0.0.1/api/comments', {
      headers: { Host: '127.0.0.1', Origin: 'http://127.0.0.1', [SESSION_TOKEN_HEADER]: TOKEN },
    }))
  ...
  it('sets no-store/nosniff/no-referrer on authenticated HTML and API responses', async () => {
    const responses = [
      await app.fetch(new Request('http://127.0.0.1/', {
        headers: { Host: '127.0.0.1', [SESSION_TOKEN_HEADER]: TOKEN },
      })),
      await app.fetch(new Request('http://127.0.0.1/api/comments', {
        headers: { Host: '127.0.0.1', [SESSION_TOKEN_HEADER]: TOKEN },
      })),
    ]
