...
async function seedSuggestionComment(store: InMemoryCommentStore): Promise<void> {
...
beforeEach(() => {
  ...
  spies.write.mockImplementation(
    async (_path: string, bytes: Buffer) => ({
      sha256: hash(bytes),
      size: bytes.length,
    }),
  )
...
describe('POST /api/save-file', () => {
  ...
  describe.each([
    { name: 'non-string filePath', payload: { filePath: 42, content: 'new\n' } },
    { name: 'empty-string filePath', payload: { filePath: '', content: 'new\n' } },
    { name: 'non-string content', payload: { filePath: 'a.txt', content: 123 } },
    { name: 'non-boolean gitAdd', payload: { filePath: 'a.txt', content: 'new\n', gitAdd: 'yes' } },
...
describe('GET /api/file-content and /api/file-text native errors', () => {
  ...
  it.each([
    { url: '/api/file-content?path=a.txt&version=middle', name: 'file-content' },
    { url: '/api/file-text?path=a.txt&version=middle', name: 'file-text' },
  ])('$name rejects an invalid version with 400 and reads no file', async ({ url }) => {
    const res = await get(app, url)
    expect(res.status).toBe(400)
