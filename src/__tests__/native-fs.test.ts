...
describe("NativeRepositoryFs", () => {
  ...
  beforeEach(() => {
    vi.stubEnv("DIFFING_NATIVE_FS_TEST_SENTINEL", "test-only");
    child = makeFakeChild();
    spawnCalls = [];
    let spawnCount = 0;
  ...
  describe("read", () => {
    it("decodes binary content intact and returns matching sha256", async () => {
      ...
      const bytes = Buffer.from([
        0x00, 0x01, 0xfe, 0xff, 0x80, 0x7f, 0x0a, 0x00, 0x42,
      ]);
      ...
      const options = spawnCalls[0].args[2] as { env: NodeJS.ProcessEnv };
      expect(options.env).toBeDefined();
      expect(
        Object.keys(options.env).filter((key) => key !== "SystemRoot"),
      ).toEqual([]);
      client.close();
    });
    ...
    it("reuses one child for two sequential requests", async () => {
      ...
      expect(await first).toMatchObject({ bytes: Buffer.from("one") });
      ...
      expect(await second).toMatchObject({ bytes: Buffer.from("two") });
    ...
    it("two replies coalesced into one chunk are both delivered", async () => {
      ...
      const combined =
        JSON.stringify(
          okReply(id1, {
            contentBase64: b1.toString("base64"),
            sha256: createHash("sha256").update(b1).digest("hex"),
            size: b1.length,
          }),
        ) +
        "\n" +
        JSON.stringify(
          okReply(id2, {
            contentBase64: b2.toString("base64"),
            sha256: createHash("sha256").update(b2).digest("hex"),
            size: b2.length,
          }),
        ) +
        "\n";
    ...
    it("passes literal %2f paths through unchanged", async () => {
      ...
      child.emitReady();
      await flushAsync();
      expect((lines[0] as { op: { path: string } }).op.path).toBe(
        "weird%2fname.txt",
      );
      const id = (lines[0] as { id: number }).id;
  ...
  describe("write", () => {
    ...
    it("defaults createParents to false and omits expectedSha256", async () => {
      const client = new NativeRepositoryFs(ROOT, { binary: BINARY });
      const lines = captureStdinLines(child);
    ...
    it("snapshots the caller buffer before awaiting startup", async () => {
      ...
      expect(frame.op.contentBase64).toBe(
        Buffer.from("original bytes").toString("base64"),
      );
      const id = frame.id;
    ...
    it("rejects traversal and empty paths before spawning", async () => {
      ...
      for (const bad of ["", "../escape", "sub/../../escape"]) {
        await expect(
          client.write(bad, Buffer.from("x")),
        ).rejects.toBeInstanceOf(NativeFsError);
      }
      expect(spawnCalls.length).toBe(0);
      client.close();
    });
  ...
  describe("close and lifecycle", () => {
    ...
    it("times out a request with no reply (fake timers, rejection handled before advancing)", async () => {
      ...
      const client = new NativeRepositoryFs(ROOT, {
        binary: BINARY,
        timeoutMs: 1_000,
      });
      const lines = captureStdinLines(child);
