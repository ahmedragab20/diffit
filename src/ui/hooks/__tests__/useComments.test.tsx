  ...
  mockFetch.mockImplementation((url: string | URL, options?: RequestInit) => {
    ...
    if (
      ...
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve(sampleComments),
      });
    }
    return Promise.resolve({
      ok: true,
...
describe("useComments", () => {
  ...
  it("rejects a failed add and leaves the cached comments unchanged", async () => {
    ...
    await expect(
      result.current.addComment("x.ts", "additions", 1, "x", "body"),
  ...
  it("does not duplicate a comment when creation returns an existing id", async () => {
    ...
    mockFetch.mockImplementation((url: string | URL, options?: RequestInit) => {
      if (String(url) === "/api/comments" && options?.method === "POST") {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve(sampleComments[0]),
        });
      }
      return Promise.resolve({
        ok: true,
    ...
    expect(
      result.current.comments.filter((comment) => comment.id === "c1"),
    ).toHaveLength(1);
  });
  ...
  describe("formatAllComments", () => {
    ...
    it("formats comments as XML", async () => {
      ...
      expect(formatted).toContain(
        '<comment id="c1" line="10" side="additions" status="open" created-at="1970-01-01T00:00:01.000Z">',
      );
      ...
      expect(formatted).toContain(
        '<comment id="c2" line="5" side="deletions" status="open" created-at="1970-01-01T00:00:02.000Z">',
      );
      ...
      expect(formatted).toContain(
        '<reply id="r1" created-at="1970-01-01T00:00:03.000Z" role="agent" model="claude-3-5-sonnet">',
      );
  ...
  describe("copyAllComments", () => {
    it("copies formatted comments to clipboard", async () => {
      ...
      expect(writeText).toHaveBeenCalledWith(
        result.current.formatAllComments(),
      );
    });
  });
    ...
    it("sendToAgent forwards the verdict and an overall comment in the request body", async () => {
      ...
      await act(async () => {
        await result.current.sendToAgent(
          "changes-requested",
          "Please prioritise the security fixes",
        );
      });
      ...
      mockFetch.mockImplementation((url: string | URL, init?: RequestInit) => {
        ...
        if (urlStr === "/api/review/send") {
          ...
          return Promise.resolve({
            ok: true,
            json: () =>
              Promise.resolve({ ok: true, round: 1, openCount: 0, waiters: 0 }),
          } as unknown as Response);
        }
        return Promise.resolve({
          ok: true,
      ...
      mockFetch.mockImplementation((url: string | URL) => {
        ...
        if (urlStr === "/api/review/status") {
          return Promise.resolve({
            ok: true,
            json: () =>
              Promise.resolve({ round: 0, waiters: 1, lastSentAt: null }),
          });
        }
        return Promise.resolve({ ok: true, json: () => Promise.resolve([]) });
      });
