...
export function decideScroll({
  ...
  if (
    ...
    (viewport && (viewport.top >= viewport.height * 0.6 || viewport.bottom <= 0))
  ) {
...
export function useScrollToNextFile(_files: FileDiffMetadata[]) {
  ...
  return useCallback((path: string, opts?: ScrollToNextFileOptions) => {
    ...
    const id = next.id
    const now = Date.now()
    if (lastTarget.current?.id === id && now - lastTarget.current.at < 250) return
    lastTarget.current = { id, at: now }
    const reduce =
      opts?.reduce ?? window.matchMedia?.('(prefers-reduced-motion: reduce)').matches ?? false
    let frame = 0
