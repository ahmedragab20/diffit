...
export function formatComments(
  ...
  const sortedFilePaths = [...grouped.keys()].sort((a, b) =>
    a.localeCompare(b),
  );

  const modeAttr =
    mode && mode !== "standard" ? ` mode="${escapeAttr(mode)}"` : "";
  const lines: string[] = [];
  lines.push(
    decision
      ? `<code-review-comments decision="${escapeAttr(decision)}"${modeAttr}>`
      : `<code-review-comments${modeAttr}>`,
  );
  ...
  if (mode === "comment-only") {
    lines.push(
      "    ⚠️ COMMENT-ONLY MODE: You MUST NOT edit any files. Your only task is to reply to the comments below — answer questions, provide clarification, or discuss. The general comment (if present) is your prompt for this chat.",
    );
  } else {
    lines.push(
  ...
  if (decision) {
    lines.push(
      '    - The "decision" attribute on the root element is the reviewer\'s headline verdict: "approved", "changes-requested", "rejected", or "comment-only".',
    );
    lines.push(
  ...
  if (mode) {
    lines.push(
      `    - The "mode" attribute on the root element controls your behavior: "standard" (apply edits) or "comment-only" (reply only, no edits).`,
    );
  }
  lines.push(
  ...
  lines.push(
    '    - When line="A-B", the range is INCLUSIVE on that side (both A and B and every line between). Address the whole span, not only the last line.',
  );
  lines.push(
  ...
  lines.push(
    "        nit = optional polish; question = needs an answer; praise = positive (no change required).",
  );
  lines.push(
    '      Omit severity (or severity="none") means untriaged — treat as a normal request.',
  );
  lines.push(
  ...
  if (decision) {
    lines.push(
      `  <decision-summary><![CDATA[${reviewDecisionSummary(decision)}]]></decision-summary>`,
    );
  }
  ...
  for (const filePath of sortedFilePaths) {
    ...
    for (const comment of fileComments) {
      ...
      if (comment.lineNumber !== 0) {
        ...
        const isMultiLine =
          comment.lineContent && comment.lineContent.includes("\n");
        let codeVal = "";
        if (isMultiLine) {
        ...
        lines.push(
          `      <code><![CDATA[${escapeCdataText(codeVal)}]]></code>`,
        );
      }
      lines.push(
        `      <body><![CDATA[${escapeCdataText(comment.body)}]]></body>`,
      );
