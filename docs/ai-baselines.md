...
Artifacts are written to `~/.diffing/ai-browser/<first12 SHA256(cwd)>/report.json`, with test results in the sibling `results/` directory. The report records 20 keydowns per case to the second animation frame (not confirmed paint), p50/p95, long tasks during loaded-component→typing→reopen, scroll/draft restoration, viewport, and computed motion. Observed local values (latest `pnpm test:ai:e2e` on this branch): keydown→second-rAF p50 14–19ms, p95 25–32ms across diff/PR-diff/plan × desktop/narrow/reduced-motion; sampled JS heap about 27–35MB used. Typing preserved scroll 0, drafts restored, zero unknown requests. These are observations, not approved acceptance gates; timing is noisy and is not hardcoded as CI thresholds. `humanApprovedThresholds` stays false. Browser metrics live in this separate report; `bench:ai` still reports `browserMeasurements` null.
